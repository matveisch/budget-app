...
const SingleTransaction = (item: Transaction, index: number) => {
    ...
    return (
        <tr key={index} id={String(item.id)} className='bg-white border-b dark:bg-gray-800 dark:border-gray-700'>
            ...
            <td className='py-4 px-6'><button onClick={handleEdit}>{editMode ? 'save' : 'edit'}</button></td>
            <td className='py-4 px-6'><button onClick={() => deleteTransaction(item.id)}>delete</button></td>
        </tr>
    );
};
